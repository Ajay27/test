...
Suite Setup  setup_add_new_host
Force Tags  Host
*** Variables ***
...
sdp.1001
      [Template]  validate_check
        ...
        \   ${host.hostclass.network_name}  ${data}
sdp.1002
...
validate_check
    [Arguments]  ${field}  ${data}
     log  ${field} ${data}
     create_new_host  ${driver}
     validate  ${driver}    ${field}    ${data}

validate_positive_host
    [Arguments]  ${field}  ${data}  ${pagename}
    log  ${field} ${data}
    create_new_host  ${driver}
    host_type
    HW_Rev  ${driver} hostclass
